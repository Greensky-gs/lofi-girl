...
export default new ButtonHandler({
    ...
    button.showModal(new ModalBuilder()
    .setTitle("Comment")
    .setCustomId('modal-comment')
    .setComponents(
        row<TextInputBuilder>(new TextInputBuilder().setLabel("Comment").setStyle(TextInputStyle.Paragraph).setMaxLength(1000).setRequired(true).setPlaceholder(`Write your comment about ${station.emoji} ${station.name} here`))
    )
    )
})