...
export default new ButtonHandler({
    ...
    await Promise.all([button.showModal(
        new ModalBuilder()
            .setTitle('Comment')
        ...
        message.edit({
            components: [
                row(...buttons.map(x => new ButtonBuilder(x.toJSON()).setDisabled(true)))
            ]
        })
    ]);
    ...
    if (modal) {
        ...
        message.edit({
            components: [
                row(...buttons.map(x => new ButtonBuilder(x).setDisabled(false)))
            ]
        }).catch(() => {});
    }
});
