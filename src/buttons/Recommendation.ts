...
import { ComponentType, Message, ModalBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from "discord.js";
...
export default new ButtonHandler({
    ...
    const stations = configs.stations.filter(x => x.name.toLowerCase().includes(stationName) || stationName.toLowerCase().includes(x.name))
    ...
    const msg = await modal.deferReply({
        fetchReply: true
        ...
        await modal.editReply({
            ...
            components: [
                row<StringSelectMenuBuilder>(new StringSelectMenuBuilder()
                    ...
                    .setMaxValues(1)
                    .setOptions(
                        ...[...stations.splice(0, 24).map(st => ({ label: st.name, emoji: st.emoji, value: st.url, description: `Station ${resizeStr(st.name, 80)}` })), { label: 'Cancel', value: 'cancel', description: 'Cancel the selection', emoji: '❌' }]
                    )
