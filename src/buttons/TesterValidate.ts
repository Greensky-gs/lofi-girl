...
export default new ButtonHandler({
    ...
    if (!channel) return button.editReply({
    ...
    const msg = await channel.send({
        embeds: [
            new EmbedBuilder()
                ...
                .setColor('Orange')
                .setDescription(`${user.username} just sent this feedback about [${station.emoji} ${station.name}](${station.url})`)
                .setFields(
                    {
                        name: 'Comment',
    ...
    button.editReply({
        content: msg ? `Your feedback has been sent to the owner` : `Your feedback hasn't been sent to the owner because of an internal error`
    }).catch(() => {});
