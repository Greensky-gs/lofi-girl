...
export default new LofiCommand({
    ...
    execute: async({ interaction, options }) => {
        ...
        if (queue.nowPlaying().duration === '0:00' || queue.tracks[queue.tracks.length - 1].duration === '0:00') return interaction.reply(`:x: | You can't add a station to play it after a station (live)`).catch(() => {});
        ...
        if (!track || track.tracks.length === 0) return interaction.editReply(getRep(`:x: | Music station not found`)).catch(() => {});
