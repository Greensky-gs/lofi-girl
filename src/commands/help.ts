...
export default new AmethystCommand({
    ...
    const cmd: AmethystCommand = interaction.client.chatInputCommands.find(x => x.options.name === options.getString('command'));
    if (cmd) {
        return interaction.reply({
            embeds: [ new EmbedBuilder()
                ...
                .setFields(
                    {
                        name: 'Permissions',
                        value: cmd.options.preconditions?.find(x => x.name === 'adminIfNotAlone') ? 'Administrator if user is not alone in the channel' : '',
                        inline: false
                    },
                    {
                        name: 'Usable in direct messages',
                        value: boolEmojis(!(cmd.options.preconditions?.find(x => x.name === 'GuildOnly') ?? false)),
                        inline: true
                    }
                )
                .setColor('Orange')
