...
export default new AmethystCommand({
    ...
    const formatTime = (key: string) =>
        key.split(':').length === 2
            ? parseInt(key.split(':')[0]) * 60 + parseInt(key.split(':')[1])
            : parseInt(key.split(':')[0]) * 36000 + parseInt(key.split(':')[1]) * 60 + parseInt(key.split(':')[2]);
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            interaction.client.langs.getText(
                interaction,
                'playing',
                playingTrack ? 'descriptionWithTrack' : 'description',
                {
                    stationName: station.name,
                    stationEmoji: station.emoji,
                    stationUrl: station.url,
                    trackName: playingTrack
                        ? playingTrack.includes('-')
                            ? playingTrack.split('-')[1]
                            : playingTrack
                        : ''
                }
            )
