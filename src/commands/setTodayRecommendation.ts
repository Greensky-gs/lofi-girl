...
export default new AmethystCommand({
    ...
    if (!station)
        return message.channel.send(
            `:x: | This station doesn't exist. Take for exemple this : [${randomised.emoji} ${randomised.name}](<${station.url}>)`
        );

    if ((configs.recommendation as station)?.url === station.url && options.second !== '-f') {
        return message.channel.send(
            `:x: | This is the recommendation of the day. Use \`${message.client.configs.prefix}recommendation <station url> -f\` to set it anyway`
        );
    }
    ...
    message.channel.send(`❤️ | Set ${station.emoji} ${station.name} as the recommendation of the day`);
});
