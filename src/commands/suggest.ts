...
.setChatInputRun(async({ interaction, options }) => {
    ...
    if (!info || info.videoDetails.author.id !== lofiGirlID) return interaction.editReply(`:x: | You can suggest only videos from [Lofi Girl](https://youtube.com/c/LofiGirl) channel`).catch(() => {});

    const station = getStationByUrl(url, false);
    if (station) return interaction.editReply(`${station.emoji} | This song already exists in my song list`).catch(() => {});
