...
export default new LofiCommand({
    ...
    execute: async({ interaction, options }) => {
        ...
        if (queue.connection.channel.members.filter(m => !m.user.bot).size >= 2 && !(interaction.member as GuildMember).permissions.has('Administrator')) return interaction.reply(`:x: | Since you're not alone in the channel, only administrators can modify volume`).catch(() => {});
