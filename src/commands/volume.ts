...
export default new LofiCommand({
    ...
    execute: async({ interaction, options }) => {
        ...
        if (queue.channel.members.filter(x => !x.user.bot).size > 1 && !interaction.member.permissions.has('Administrator')) return interaction.reply(`:x: | Since you're not alone in the channel, only administrators can change the volume`).catch(() => {});
