...
export default new AmethystEvent('commandDenied', (command, reason) => {
    const includedReplies = [
        ...
        { x: commandDeniedCode.UserMissingPerms, y: `You need ${reason.metadata.permissions.need.length} permissions to run this command. You have ${reason.metadata?.permissions?.got?.length}` },
        { x: commandDeniedCode.ClientMissingPerms, y: `I need ${reason.metadata.permissions.need.length} permissions to run this command. I have ${reason.metadata.permissions.got.length}` }
    ]
