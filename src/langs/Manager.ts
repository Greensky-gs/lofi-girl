...
export class Langs {
    private langs: Record<string, {
        commands: Record<string, command>,
        texts: Record<string, Record<string, string>>,
        buttons: Record<string, { label?: string; emoji?: string }>,
        selectors: Record<string, { placeholder?: string; content?: string; options?: { name?: string; emoji?: string; }[] }>
    }> = {
        fr: imports.fr,
        en: imports.en
    };
    ...
    public buildLocalizations<Command extends keyof localizationsType<'commands'>>(command: Command): localizationBuilder<Command> {
        const names = {} as any;
            ...
            if (locale === 'en') {
                ['en-US', 'en-GB'].forEach((loc) => {
                    ...
                    Object.keys(imports[locale].commands[command].options).forEach((optionName) => {
                        ...
                        options[optionName].description[loc] = imports[locale].commands[command].options[optionName].description
                    })
                })
            } else {
                ...
                Object.keys(imports[locale].commands[command].options).forEach((optionName) => {
                    ...
                    options[optionName].description[locale] = imports[locale].commands[command].options[optionName].description
                })
            }
        })
    ...
    public getButton(interaction: BaseInteraction, button: keyof localizationsType<'buttons'>, data: { id?: string; emoji?: string; url?: string; style: keyof typeof ButtonStyle }) {
    ...
    public getSelectorData<Selector extends keyof localizationsType<'selectors'>>(interaction: BaseInteraction, selector: Selector): localizationsType<'selectors'>[Selector] {
